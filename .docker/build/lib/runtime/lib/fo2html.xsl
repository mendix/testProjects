...
<xsl:template match="fo:table">
  ...
  <table>&add-style;
    ...
    <xsl:if test="@padding">
      ...
      <xsl:if test="@border-spacing">
      <xsl:attribute name="cellspacing">
        <xsl:value-of select="@border-spacing"/>
        </xsl:attribute>
    </xsl:if>
...
<xsl:template match="fo:external-graphic">
  ...
  <xsl:apply-templates select="@height|@width|@*[starts-with(name(),'border')]"/></img>
</xsl:template>
...
<xsl:template match="*[@white-space-collapse='false'
                    ...
                    or @white-space='pre']"
                     mode="check-for-pre">
  <pre style="white-space: pre-wrap;"><xsl:apply-templates select="."/></pre>
</xsl:template>
