...
(function(
	...
	if( 1 ){
		...
		if(has("dojo-cdn") ||  1 ){
			...
			insertPointSibling = Array.from(scripts).find(s => 
				s.src && 
				!s.src.match(/^(\w+-)*extension:/) &&
				!s.src.match(/^webkit-masked-url:/)
			);
		}
	...
	if( 1  && ( 1  ||  1 )){
		...
		if( 1 ){
			...
			while(!insertPointSibling){
				if(
					!/^dojo/.test((script = scripts[i++]) &&
					script.type &&
					script.src &&
					!script.src.match(/^(\w+-)*extension:/) &&
					!script.src.match(/^webkit-masked-url:/))
				){
					insertPointSibling= script;
				}
			}
